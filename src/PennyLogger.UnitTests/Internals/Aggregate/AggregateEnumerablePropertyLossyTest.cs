...
{
    /// <summary>
    /// Unit tests for <see cref="AggregateEnumerablePropertyLossy{T}"/>
    /// </summary>
    public class AggregateEnumerablePropertyLossyTest
    {
        private AggregateEnumerablePropertyLossy<string> Create()
        {
            TestHelpers.BuildConstructorParameters(out var prop, out var config);
            return new AggregateEnumerablePropertyLossy<string>(prop, config);
        }
